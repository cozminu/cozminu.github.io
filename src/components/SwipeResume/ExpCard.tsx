    ...
    <div className="flex flex-col h-full bg-slate-950 text-white rounded-3xl overflow-hidden relative p-8">
      ...
      <div className="relative z-10 mb-6 border-b border-white/10 pb-4">
        <div className="flex items-center justify-between mb-2">
          <span className="px-3 py-1 rounded-full bg-indigo-500/20 text-indigo-400 text-[10px] font-bold tracking-widest uppercase border border-indigo-500/30">
            Experience
          </span>
        ...
        <div className="flex items-center text-slate-400 text-sm font-medium">
          <Building size={14} className="mr-1.5 text-indigo-400" />
          <span>{data.name}</span>
        </div>
      </div>
      ...
      <div className="relative z-10 flex-1 space-y-4">
        {data.highlights.map((highlight, i) => (
          ...
          >
            <div className="min-w-[4px] h-[4px] mt-2 rounded-full bg-indigo-500 shadow-[0_0_8px_rgba(99,102,241,0.6)]" />
      ...
      <div className="relative z-10 mt-auto pt-6">
        {data.technologies && (
          <div className="flex flex-wrap gap-2 mb-4">
            {data.technologies.map((tech) => (
              <span
                key={tech}
                className="px-2.5 py-1 text-[11px] font-medium rounded-md bg-white/5 border border-white/10 text-slate-400"
              >
                {tech}
              </span>
            ))}
        ...
        {data.pastExperience && data.pastExperience.count > 0 && (
          <div className="flex items-center pt-4 border-t border-white/5">
            <div className="flex items-center justify-center w-8 h-8 rounded-full bg-slate-800 text-indigo-400 mr-3">
              <Briefcase size={14} />
            </div>
